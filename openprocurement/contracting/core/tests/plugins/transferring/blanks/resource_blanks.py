...
def change_resource_ownership(self):
    change_ownership_url = '/contracts/{}/ownership'
    post = self.app.post_json

    req_data = {"data": {"id": 1984}}
    response = post(change_ownership_url.format(self.contract_id), req_data, status=422)
    ...
    contract = self.get_contract(self.contract_id)
    self.assertEqual(contract['data']['owner'], self.first_owner)
    ...
    req_data = {"data": {"id": transfer['data']['id'],
                         'transfer': self.contract_transfer}}

    response = post(change_ownership_url.format(self.contract_id), req_data)
...
def already_applied_transfer(self):
    ...
    contract_wich_use_transfer = self.create_contract()
    contract_wich_want_to_use_transfer = self.create_contract()
    transfer = self.create_transfer()
    self.use_transfer(transfer,
                      contract_wich_use_transfer['data']['id'],
                      contract_wich_use_transfer['access']['transfer'])

    req_data = {"data": {"id": transfer['data']['id'],
                         'transfer': contract_wich_want_to_use_transfer['access']['transfer']}}
    req_url = '/contracts/{}/ownership'.format(contract_wich_want_to_use_transfer['data']['id'])
...
def half_applied_transfer(self):
    ...
    transfer_doc['usedFor'] = '/contracts/' + contract['data']['id']
    self.db.save(transfer_doc)

    self.use_transfer(transfer,
                      contract['data']['id'],
                      contract['access']['transfer'])

    contract_wich_used_transfer = self.get_contract(contract['data']['id'])
    self.assertEqual(self.second_owner, contract_wich_used_transfer['data']['owner'])
...
def new_owner_can_change(self):
    ...
    # second_owner can change the contract
    desc = "second_owner now can change the tender"
    req_data = {"data": {"description": desc}}
    req_url = '/contracts/{}?acc_token={}'.format(contract['data']['id'], new_access_token)
...
def old_owner_cant_change(self):
    ...
    # fist_owner can`t change the contract
    desc = "make contract greate again"
    req_data = {"data": {"description": desc}}
    req_url = '/contracts/{}?acc_token={}'.format(
        contract['data']['id'], contract['access']['token']
    )
...
def broker_not_accreditation_level(self):
    ...
    req_data = {"data": {"id": transfer['data']['id'],
                         'transfer': self.contract_transfer}}
    req_url = '/contracts/{}/ownership'.format(self.contract_id)
...
def switch_mode(self):
    ...
    self.set_contract_mode(contract['data']['id'], 'test')

    self.app.authorization = ('Basic', (self.test_owner, ''))
    transfer = self.create_transfer()

    req_data = {"data": {"id": transfer['data']['id'],
                         'transfer': contract['access']['transfer']}}
    req_url = '/contracts/{}/ownership'.format(contract['data']['id'])
...
def create_contract_by_concierge(self):
    ...
    self.assertEqual(response.content_type, 'application/json')
    self.assertEqual(
        response.json['data']['usedFor'], '/contracts/{}'.format(contract_id)
    )
